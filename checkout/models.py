    ...
    def update_subtotal(self):
        ...
        self.order_subtotal = (
            self.lineitems.aggregate(
                Sum("lineitem_subtotal")
                )["lineitem_subtotal__sum"] or 0
        )
        if self.order_subtotal > settings.DISCOUNT_THRESHOLD:
            self.discount = (
                self.order_subtotal * settings.DISCOUNT_PERCENTAGE / 100
            )
        else:
            self.discount = 0
